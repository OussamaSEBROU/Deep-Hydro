...
from streamlit_oauth import OAuth2Component # Import OAuth component

# --- Load Environment Variables & Initialize Services ---
load_dotenv() # Load .env for local development first

# Store initialization status and errors
...
def initialize_services():
    """Initialize Firebase, Gemini, OAuth, Admin Pass from environment variables."""
    global firebase_initialized, gemini_configured, google_oauth_configured, admin_password, service_init_errors
    global CLIENT_ID, CLIENT_SECRET, REDIRECT_URI # Ensure globals are referenced
    ...
    if not firebase_admin._apps:
        try:
            ...
            if firebase_creds_json:
                ...
                if not project_id:
                    service_init_errors.append("Firebase Service Account JSON missing project_id.")
                else:
                    firebase_url = os.getenv("FIREBASE_DATABASE_URL", f"https://{project_id}-default-rtdb.firebaseio.com/")
                    firebase_admin.initialize_app(cred, {"databaseURL": firebase_url})
                    firebase_initialized = True
            else:
                service_init_errors.append("FIREBASE_SERVICE_ACCOUNT env var not found.")
    ...
    if GEMINI_API_KEY:
        ...
        service_init_errors.append("GOOGLE_API_KEY env var not found.")
        gemini_configured = False

    # --- Google OAuth --- 
    # Reload from env vars in case they were updated
    CLIENT_ID = os.getenv("GOOGLE_CLIENT_ID")
    CLIENT_SECRET = os.getenv("GOOGLE_CLIENT_SECRET")
    REDIRECT_URI = os.getenv("REDIRECT_URI")
    if all([CLIENT_ID, CLIENT_SECRET, REDIRECT_URI]):
        ...
        service_init_errors.append(f"Google OAuth env vars missing: {', '.join(missing_vars)}.")
        google_oauth_configured = False
...
# Display service init errors if any (AFTER set_page_config)
...
def get_persistent_user_id():
    ...
    # If logged in via Google OAuth, use the email as ID
    if st.session_state.get("auth_status") == "authenticated" and st.session_state.get("user_email"):
        user_id = st.session_state.user_email
        # Ensure persistent_user_id is updated if it was previously anonymous
        if st.session_state.get("persistent_user_id") != user_id:
             st.session_state.persistent_user_id = user_id
        return user_id
    # If already have a persistent ID (could be anon or email from previous session), use it
...
def update_firebase_profile_on_login(email):
    ...
    try:
        ...
        update_data = {
            ...
            "user_id": email # Ensure user_id is the email
        }
        if profile is None:
        ...
        if profile is None:
            ...
            if "session_visit_logged" not in st.session_state:
                ...
                if current_auth_status:
                     ...
                     # Keep existing email if user logs out during session
...
def show_google_login():
    ...
    # Check if user needs to log in (limit reached and not authenticated)
    access_needed, reason = check_feature_access()
    show_login_button = not access_needed and st.session_state.get("auth_status") != "authenticated"

    if "token" not in st.session_state:
        if show_login_button:
            st.sidebar.warning(reason) # Show reason like "Usage limit reached"
            ...
            if result:
                ...
                if user_info and user_info.get("email"):
                    ...
                    st.session_state.persistent_user_id = user_info.get("email") # Update persistent ID
        ...
        # Verify token is still valid and get user info if needed
        if not st.session_state.get("user_email"):
             ...
             if user_info and user_info.get("email"):
                 ...
                 st.session_state.persistent_user_id = None # Reset persistent ID
...
**Config Note:** This version reads config from separate environment variables (e.g., `FIREBASE_SERVICE_ACCOUNT`, `GOOGLE_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `REDIRECT_URI`, `ADMIN_PASSWORD`). Ensure these are set securely in your deployment.
</div>
""", unsafe_allow_html=True)
